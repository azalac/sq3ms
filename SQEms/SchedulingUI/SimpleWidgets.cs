    ...
    public class Box : Component
    {
        ...
        public override void Draw(IConsole buffer)
        {
            ...
            for (int x = Left; x < Left + Width; x++)
            {
                if (x == Left)
                {
                    buffer.PutCharacter(UTOP_LEFT);
                }
                else if (x == Left + Width - 1)
                {
                    buffer.PutCharacter(UTOP_RIGHT);
                }
                else
                {
                    buffer.PutCharacter(UHORTIZONTAL);
                }
            }

            for (int y = Top + 1; y < Top + Height - 1; y++)
            {
                buffer.PutCharacter(Left, y, UVERTICAL);
                buffer.PutCharacter(Left + Width - 1, y, UVERTICAL);
            }
            ...
            for (int x = Left; x < Left + Width; x++)
            {
                if (x == Left)
                {
                    buffer.PutCharacter(UBOTTOM_LEFT);
                }
                else if (x == Left + Width - 1)
                {
                    buffer.PutCharacter(UBOTTOM_RIGHT);
                }
                else
                {
                    buffer.PutCharacter(UHORTIZONTAL);
